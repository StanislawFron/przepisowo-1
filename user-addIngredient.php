<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $sql ='ALTER TABLE przepisy ADD skl'.$newIngredientNumber.' VARCHAR(45) NOT NULL DEFAULT "empty"';
        $statement = $pdo->prepare($sql);
        $statement->execute();

        $sql ='ALTER TABLE przepisy ADD skl'.$newIngredientNumber.'Am VARCHAR(45) NOT NULL DEFAULT ""';
        $statement = $pdo->prepare($sql);
        $statement->execute();
